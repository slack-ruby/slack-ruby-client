  ...
  namespace :real_time do
    namespace :api do
      ...
      task update: [:git_update] do
        ...
        File.write 'lib/slack/real_time/store/event_handlers.rb', event_handlers_template.result(events: events.keys)
        # each event handler
        ...
        events.each_pair do |name, json|
          event_handler_filename = "lib/slack/real_time/store/event_handlers/#{name}.rb"
