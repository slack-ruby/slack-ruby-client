...
* [#596](https://github.com/slack-ruby/slack-ruby-client/pull/596): Fix the `update_api` workflow failing with exit code 5 when the AI-generated changelog response isn't valid JSON, extract the JSON answer from Copilot CLI's response even when preceded by narration/thinking text, and always log the raw AI response for debugging - [@dblock](https://github.com/dblock).
