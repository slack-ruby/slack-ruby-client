...
describe Slack::Messages::Formatting do
  ...
  context '#unescape' do
    ...
    it 'unescapes a user reference' do
      expect(subject.unescape('Hey <@U024BE7LH|bob>, did you see my file?')).to(
    ...
    it 'unescapes a URL without text' do
      expect(subject.unescape('This message contains a URL <http://foo.com/>')).to(
        eq('This message contains a URL http://foo.com/')
      )
    end
    it 'unescapes a URL with text' do
      expect(subject.unescape('So does this one: <http://www.foo.com|www.foo.com>')).to(
    ...
    it 'unlinkifies references' do
      expect(subject.unescape('Hello <@U123|bob>, say hi to <!everyone> in <#C1234|general>')).to(
